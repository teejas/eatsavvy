...
func (v *VapiClient) CreateCall(restaurant places.Restaurant) (VapiCallResponse, error) {
	reqBody := vapi.GetAssistantRequestBody(restaurant)
	...
	if err != nil {
		slog.Error("[vapi.makeVapiPhoneCall] Failed to send HTTP request", "error", err)
		return VapiCallResponse{}, err
	}
	if statusCode >= 400 {
		slog.Error("[vapi.makeVapiPhoneCall] Failed to create Vapi call", "statusCode", statusCode, "responseBody", string(respBody))
	...
	if err != nil {
		slog.Error("[vapi.makeVapiPhoneCall] Failed to unmarshal response body", "error", err)
		return VapiCallResponse{}, err
	}
